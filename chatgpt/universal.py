...
async def handle_message(_respond: Callable, session_id: str, message: str,
    ...
    def wrap_request(n, m,queue_info):
        """
        Wrapping send messages
        """
        async def call(session_id, message, conversation_context, respond,queue_info):
            await m.handle_request(session_id, message, respond, conversation_context, n,queue_info)
    ...
    async def request(_session_id, prompt: str, conversation_context, _respond,queue_info = None):
        """
        Request method
        """
    ...
    # 不带前缀 - 正常初始化会话
        ...
        # 没有任务那就聊天吧！
        if not task:
            task = conversation_context.ask(prompt=prompt, chain=chain, name=nickname,queue_info = queue_info)
    ...
    try:
        ...
        queue_info = None
        for m in middlewares:
            action = wrap_request(action, m,queue_info)

        # 开始处理
        await action(session_id, message.strip(), conversation_context, respond,queue_info)
