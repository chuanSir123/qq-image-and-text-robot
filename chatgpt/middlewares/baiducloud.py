...
class BaiduCloud:
    ...
    async def get_conclusion_image(self, text: str):
        await self.check_and_update_access_token()

        baidu_url = f"https://aip.baidubce.com/rest/2.0/solution/v1/img_censor/v2/user_defined" \
                    f"?access_token={self.access_token}"
        headers = {'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json'}
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            try:
                async with session.post(baidu_url, headers=headers, data={'image': text}) as response:
                    response.raise_for_status()
                    response_dict = await response.json()
                    return response_dict
...
class MiddlewareBaiduCloud(Middleware):
    ...
    async def handle_respond(self, session_id: str, prompt: str, rendered: str, respond: Callable, action: Callable):
        ...
        try:
            ...
            if isinstance(rendered, Image):
                response_dict = await self.baidu_cloud.get_conclusion_image(rendered)
            else:
