...
class Onebot(BaseModel):
    ...
    qq_nick_name: str
    """机器人管理员的 QQ 号"""
    reverse_ws_host: str = "127.0.0.1"
    """go-cqhttp 的 反向 ws 主机号"""
    reverse_ws_port: Optional[int] = 8568
...
class OpenAIAuths(BaseModel):
    browserless_endpoint: Optional[str] = "https://gpt.pawan.krd/backend-api/"
    """自定义无浏览器登录模式的接入点"""
...
class SlackAppAccessToken(BaseModel):
    ...
    cookie: str
    organization_id: str
    claude2_linshi_email_id: str = "2fe596ba92df7da0da567b5f5c0934ee"
    ...
    #app_endpoint: str = "https://h5hmcq.laf.dev/"
    """API 的接入点"""


class SlackAuths(BaseModel):
    claude2: bool=False
    claude2_current_account: int=1
    slack_max_token: int=4000
    slack_token: str=""
    slack_cookie: str=""
    slack_team_id: str=""
    slack_channel: str=""
    slack_claude_user: str=""
    slack_main_last: bool=True
    slack_ping_message: str=""
    third_slack_ping_message: str=""
...
class Response(BaseModel):
    ...
    sillyClaudeIdList: List[str] = ["416448943"]

    limitRoleplayIdList:  List[str] = []

    generateCurrentImage: bool = True
...
class SDWebUI(BaseModel):
    api_url_free: str
    default_trans: str = 'bing'
    default_sd_model: str
    default_sd_vae: str
    default_real_sd_model: str
