    ...
    def __truncate_conversation(self, convo_id: str = "default") -> None:
        ...
        while True:
            if (
                ...
                break
    def __truncate_conversation1(self, messages) -> None:
        """
        Truncate the conversation
        """
        while True:
            if (
                    self.get_token_count1(messages) > self.truncate_limit
                    and len(messages) > 1
            ):
                # Don't remove the first message
                messages.pop(2)
                ...
                break
    def get_token_count1(self, messages) -> int:
        """
        Get token count
        """
        ...
        async with self.aclient.stream(
            ...
            async for line in response.aiter_lines():
                ...
                if "content" in delta:
                    ...
                    yield content
        self.add_to_conversation(full_response, response_role, convo_id=convo_id)
    async def ask_stream_async1(
            self,
            messages,
            **kwargs,
    ) -> AsyncGenerator[str, None]:
        self.__truncate_conversation1(messages)
        # Get response
        async with self.aclient.stream(
                "post",
                os.environ.get("API_URL") or "https://api.openai.com/v1/chat/completions",
